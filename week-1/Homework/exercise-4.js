...
let restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function (numberOfPeople) {
    return this.restaurants.filter((rest) => rest.totalSeats - rest.numberOfCustomers >= numberOfPeople).map(rest => rest.name)
  },
  findRestaurantServingDish: function (dishName) {
    return this.restaurants.filter(rest => rest.menu.includes(dishName)).map(rest => rest.name);
  },
  countNumberOfRestaurantsInArea: function (areaa) {
    return this.restaurants.filter(rest => rest.address.area === areaa).length
  }
};
